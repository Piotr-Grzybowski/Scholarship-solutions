...
function transformFile(inputFilePath, outputFilePath) {
    pipeline(fs.createReadStream(inputFilePath, {
        encoding: "UTF-8",
        highWaterMark: 10 * 1024 * 1024,
    }), splitChunkOnCellsFactory(), addColumnFactory(), fs.createWriteStream(outputFilePath))
        .then(() => {
        console.log(`File ${inputFilePath} has been transformed into file ${outputFilePath} successfully!`);
    })
        .catch((error) => {
        console.error(error);
    });
