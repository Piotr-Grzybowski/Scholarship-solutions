...
class GroupOfSatellites {
    ...
    deleteSatellite(satellite) {
        if (this.findSatellite(satellite))
            this.listOfSatellites.splice((0, getIndex_1.getIndex)(this.listOfSatellites, satellite.uuid), 1);
        else
            throw new Error("There is no such satellite in this group!");
    }
    findSatellite(satellite) {
        const index = (0, getIndex_1.getIndex)(this.listOfSatellites, satellite.uuid);
        if (index !== -1)
            return satellite;
        return false;
    }
